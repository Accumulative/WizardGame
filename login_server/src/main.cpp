						...
						switch(tmp[0]) {
							case '1': {
								...
								while (!done) {
									...
									case SQLITE_ROW:
										db_password = strdup((const char*)sqlite3_column_text(stmt, 2));
								...
								if(db_password) {
									if(strcmp(db_password, password) == 0) {
										sprintf(tmp, "1 0 %d %d %d \n", data.map_id, data.xpos, data.ypos);
							...
							case '3': {
								...
								snprintf(
												q,
												sizeof q - 1,
												"UPDATE users SET map_id='%d', x_position='%d', y_position='%d' where username='%s';",

...
struct data {
	...
	int mapId;
	Vector2D pos;
	data(TCPsocket sock, Uint32 t, int m, Vector2D pos, int i) : socket(sock), timeout(t), mapId(m), pos(pos), id(i)
	{
...
int main(int argc, char** argv) {
	...
	SDL_Window *window = SDL_CreateWindow("title", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 600, 400, NULL);
	...
	while(running) {
		...
		if(tmpsocket) {
			if(playernum < 30) {
				...
				socketvector.push_back(data(tmpsocket, SDL_GetTicks(), 1, Vector2D(515.0f, 709.0f), curid));
				playernum++;
				sprintf(tmp, "0 %d \n", curid);
